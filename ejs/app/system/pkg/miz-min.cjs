...
async function getPathsConfig(projectType = "default") {
	...
	return {
		...
		resetSassFile: path.join(mizRoot, "views", "base", "_reset.scss"),
		resetCssFilePath: path.join(mizRoot, "system", "sass", "kernel", "responsive", "mixins", "_reset.scss"),
	}
}
...
async function findAllFiles(directory, ignoredPaths = [], validExt = [".html"]) {
	const results = []
...
const CLASS_PATTERNS = [/class\s*=\s*'([^']+)'/g, /class\s*=\s*"([^"]+)"/g, /className\s*=\s*'([^']+)'/g, /className\s*=\s*"([^"]+)"/g, /querySelector\(["']([^"']+)["']\)/g, /querySelectorAll\(["']([^"']+)["']\)/g, /classList\.remove\(['"]([^'"]+)['"]\)/g, /classList\.add\(['"]([^'"]+)['"]\)/g, /classList\.contains\(['"]([^'"]+)['"]\)/g, /classList\.toggle\(['"]([^'"]+)['"]\)/g]
...
function combineDuplicateRules(css) {
	/*
	 * =========================================================
	 * 1. Extract @import rules first
	 * =========================================================
	 */

	const imports = []

	css = css.replace(/@import\b[^;]*;/gi, (match) => {
	...
	while (i < css.length) {
		 ...
		 */
		if (atIndex > i) {
			const normal = css.slice(i, atIndex).trim()
		 ...
		 * پیدا کردن {
		 *
		 * این قسمت فقط برای at-rule هایی است
		 * که block دارند:
		 *
		 * @media
		 * @supports
		...
		if (braceIndex === -1) {
			const content = css.slice(atIndex).trim()
	...
	/*
	 * =========================================================
	 * 3. Separate normal rules and at-rules
	 * =========================================================
	 */
	 ...
	 * 5. Return final CSS
	 *
	 * @import MUST stay at the beginning.
	 * =========================================================
	 */

	return [
        ...imports,
        ...(mergedNormal ? [mergedNormal] : []),
        ...atRules,
    ].join("\n\n")
}
...
function extractAtBlocks(css, classMap) {
	const atBlocks = []
	/* * @import */ const imports = css.match(/@import\b[^;]*;/gi) || []
	atBlocks.push(...imports.map((item) => item.trim()))
	/* * @media / @supports */ const regex = /@(media|supports)[^{]+{([\s\S]*?})\s*}/g
	let match
	while ((match = regex.exec(css)) !== null) {
		const type = match[1]
		const condition = match[0].match(/@(media|supports)\s*([^{]+)/)[2].trim()
...
function extractKeyframes(cssData, classMap, animationMap) {
	...
	while ((match = regex.exec(cssData)) !== null) {
		...
		const isUsed = Array.from(classMap.keys()).some((cls) => clsHasAnimation(cls, name, animationMap))
...
function extractAtRules(cssData, classMap, animationMap, fontMap) {
	...
	return [...charsetImports, ...supportsMediaRules, ...keyframesRules, ...fontFaceRules].join("\n\n")
}
